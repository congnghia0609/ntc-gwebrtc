...
func sendMembers(s *session) error {
	...
	if membersMap, ok := room.GetRoom(s.ApiKey, s.Room); ok == true {
...
func sendSdp(s *session, raw []byte) error {
	...
	dstConn, ok := room.GetSession(s.ApiKey, s.Room, message.Args.Dst)
	if ok == false {
...
func announceExit(apiKey, room, sessionKey string) {
	...
	if membersMap, ok := room.GetRoom(apiKey, room); ok == true {
		membersMap.Range(func(key, value interface{}) bool {
...
func HandleRootWSUpgrade(w http.ResponseWriter, r *http.Request) {
	...
	defer func() {
		if err := room.DestroySession(apiKey, room, sessionKey); err != nil {
			log.Error().Err(err).
				Str("ApiKeyID", apiKey).
				Str("Room", room).
	...
	room.StoreSession(apiKey, room, sessionKey, session)
